...
source("scripts/helpers/publication_references.R")
...
file_lines <- purrr::map_chr(metadata$files, \(file) {
  glue(
    "- `data/04_curated/{file$name}` — upload as `{file$name}`: ",
    "{file$description}"
  )
})
file_lines <- c(
  file_lines,
  "- `data/06_zenodo/README.md` — upload as `README.md`: Version-specific dataset documentation and data dictionary."
)
...
zenodo_readme <- glue(
  ...
  "This README accompanies the curated CSV files deposited under the Zenodo concept DOI ",
  "https://doi.org/{metadata$zenodo$concept_doi}. It documents the tables, columns, ",
...
form <- glue(
  ...
  "## Existing resource\n\n",
  "Concept DOI: https://doi.org/{metadata$zenodo$concept_doi}\n\n",
