...
This folder holds regenerable delivery files. The folder README and the Zenodo file guide are tracked in Git; generated CSVs and archives are local. Run export scripts from the repository root after rebuilding the curated tables and reviewing QA results.

| Folder | Contents |
| --- | --- |
| `website/` | JSON files consumed by the separate Ngulia website. |
| `zenodo/` | Five dataset CSVs and a compact README, ready to upload from this folder. The builder overwrites these files. |
...
4. Review the five CSVs and `README.md` directly in `zenodo/`, plus `gbif/ngulia_gbif_dwca.zip`. Confirm that the files match the intended version and review the [interpretation limits](../docs/data_limitations.md). Enter title, creators, license, coverage, and related identifiers in the Zenodo record metadata.

## Current record status

The latest documented record checks were on **2026-09-25**. The export builders prepare files but do not publish them.

- **Zenodo:** No resolving concept DOI is recorded. The former `10.5281/zenodo.21395879` value did not resolve at that check and was removed from publication metadata. The six files to upload can be generated locally.
...
1. Create the Zenodo dataset record with the six files in `exports/zenodo/`, using [`config/publication/dataset_metadata.yml`](../config/publication/dataset_metadata.yml) for record metadata. Record the **version DOI**, concept DOI, version number, publication date, and Git commit. Analyses should cite a fixed version DOI.
